...
from models import resnet, densenet, vgg, inception #mobilenetv3, , densenet, inceptionv3,
...
def main():
    ...
    if args.model == 'res50':
        ...
        net = EfficientNet.from_pretrained('efficientnet-b5', num_classes=args.n_classes)
        ...
        net = inception.inception_v3(pretrained=True, aux_logits=False)
        net.fc = nn.Linear(2048, args.n_classes)
    print(net)
    # exit()
    ...
    if args.test:
        if args.model == 'res50':
            ...
            weight_dir = 'checkpoints/ddr_dense121_base5/28.pkl'
        elif args.model == 'mobilev3':
            ...
            weight_dir = 'checkpoints/ddr_inceptionv3_base5/79.pkl'
...
def normed_acc(confusion_matrix): # numpy confusion_matrix
    ...
    for i in range(len(confusion_matrix)):
        ...
        category_accuracy = true_positives / total_samples
...
def test(net, testloader, optimizer, epoch, test_log):
    ...
    print('normed acc: %.8f' % (normed_acc(con_matx_clf.value())))
