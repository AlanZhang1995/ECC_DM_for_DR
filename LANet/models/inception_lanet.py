...
def inception_v3(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        if 'aux_logits' in kwargs and kwargs['aux_logits'] == False:
            print('filter layer start with AuxLogits')
            state_dict = {k: v for k, v in state_dict.items() if not k.startswith("AuxLogits.")}
        msg = model.load_state_dict(state_dict, strict=False)
        if 'adaloss' in kwargs and kwargs['adaloss'] == True:
            assert set(msg.missing_keys) == {"sigma1", "sigma2", "fc_clf.weight", "fc_clf.bias", "reduce_dim.weight"}, msg
        else:
            assert set(msg.missing_keys) == {"fc_clf.weight", "fc_clf.bias", "reduce_dim.weight"}, msg
        return model
...
class Inception3(nn.Module):
    ...
    def __init__(self, num_classes=1000, aux_logits=True, transform_input=False, adaloss: bool = False):
